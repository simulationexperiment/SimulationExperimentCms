...
app.controller('myCtrl', function ($scope, $http) {
  ...
  $scope.onSearch = function () {
    location.href = '/waitingResource?systemID=' + $scope.model.selectedSystem4Search.systemID
        + '&courseID=' + $scope.model.selectedCourse4Search.courseID
        + '&resourceTypeID=' + $scope.model.selectedSourceType4Search.sourceTypeID;
  };

  $scope.onApproved = function (resourceID, resourceName) {
    bootbox.confirm({
      message: '请确认' + resourceName + '该资源审核通过？',
      buttons: {
        confirm: {
          label: '确认',
      ...
      callback: function (result) {
        if(result) {
          $http.put('/waitingResource/changeStatus', {
  ...
  $scope.onRefuse = function (resourceID, resourceName) {
    bootbox.confirm({
      message: '请确认' + resourceName + '该资源审核被拒绝？',
      buttons: {
        confirm: {
          label: '确认',
          className: 'btn-danger'
