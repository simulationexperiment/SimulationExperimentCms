...
app.controller('myCtrl', function ($scope, $http) {
  ...
  $scope.onSearch = function () {
    location.href = '/experimentReport?systemID=' + $scope.model.selectedSystem4Search.systemID
  ...
  $scope.onChange = function (reportID) {
    location.href = '/experimentReport/edit?reportID=' + reportID;
  };

  $scope.onDelete = function (reportID, reportName) {
    bootbox.confirm({
      message: '您确定要删除实验报告：' + reportName + '吗？',
      buttons: {
        confirm: {
          label: '确认',
      ...
      callback: function (result) {
        if(result) {
          $http.delete('/experimentReport?reportID=' + reportID).then(function successCallback(response) {
            if(response.data.err){
              bootbox.alert(response.data.msg);
              return false;
            }
